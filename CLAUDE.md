...
The role of this file is to describe common mistakes and confusion points that agents might encounter as they work in this project. If you ever encounter something in the project that surprises you, please alert the developer working with you and record it in this file to help prevent future agents from hitting the same thing.

This is the single instructions file for the repo. Root `AGENTS.md` is a one-line `@CLAUDE.md` import — write new guidance here, not there.
...
- When the user asks anything about **prompts** or **prompt engineering**, fetch and follow both of these before responding — the first is cross-model, the second is model-specific and takes precedence where they differ:
  - `https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/claude-prompting-best-practices`
  - `https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-opus-4-8`
- Write instructions in this file to be read literally. Current Claude models do not generalize an instruction from one item to another or infer requests you did not make, so state the scope explicitly ("apply this to every driver, not just PostgreSQL") rather than leaving it implied.
- Prefer showing the correct pattern over forbidding the wrong one — a positive example steers more reliably than a "never do X" line. Where a rule really is a prohibition, pair it with the replacement (as the Swift and concurrency sections do).
- Replace qualitative bars like "important" or "be conservative" with concrete criteria, especially in review and reporting instructions. Vague bars get followed literally and silently drop real findings.
...
- GitHub Releases are published on the **public** repo `pluk-inc/Pluk`. The release PR itself still goes to the private `pluk-inc/app-pluk`.
- A release PR is only two things: the new `CHANGELOG.md` section and the `pluk/version.xcconfig` bump. Nothing else belongs in it.
- **Do not drive releases from GitHub milestones.** Milestone titles are stale — they stopped at `v0.0.1-beta.42` (created 2026-05-16) while the app moved to the plain `0.0.N` scheme on 2026-05-12. Milestone #20's issues actually shipped in `0.0.43`, and every release since `0.0.44` was cut without a milestone. Taking the version from a milestone title would *downgrade* `MARKETING_VERSION` and break Sparkle updates.
- Resolve the next version from `pluk/version.xcconfig` instead: bump the `MARKETING_VERSION` patch and increment `CURRENT_PROJECT_VERSION` by one.
- Build the notes from PRs merged into `main` since the previous `Release <version>` commit — not from milestone issues. Link `pluk-inc/app-pluk` PR numbers.
- Leave analytics/telemetry work and internal release tooling (CI, dSYM upload, signing) out of the notes unless it ships a user-visible change.
