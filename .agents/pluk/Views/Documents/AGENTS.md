...
- Floating action bar Convex AI uses `AISearchView` -> `buildSystemPrompt(for:databaseSchema:)`, not the CMD+K prompt path
- Convex real-time table subscriptions only support browse-filter payloads for `_system/frontend/paginatedTableDocuments`; raw Convex JS queries from the floating action bar must use the one-shot fetch path instead
- Convex content tabs can render from `QueryResult` columns while `cachedSchema` is still `nil`; new-row flows must not assume schema exists, and auto-focus should skip read-only system fields like `_id`
