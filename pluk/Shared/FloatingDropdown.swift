...
struct FloatingDropdown<T: CaseIterable & Hashable>: View where T.AllCases: RandomAccessCollection, T: RawRepresentable, T.RawValue == String {
    ...
    var body: some View {
        Button(action: {
            ...
            HStack {
                Text(itemDisplay?(selection) ?? selection.rawValue)
                Spacer()
