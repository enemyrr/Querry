...
final class WorkOSAuthService {
    ...
    func login(signUp: Bool = false, source: String = "unknown") {
        ...
        guard let clientId = WorkOSConfig.clientId else {
            isLoading = false
            authError = "Authentication is not configured for this build."
    ...
    private func refreshAccessToken() async -> Result<Void, TokenRefreshFailureReason> {
        ...
        guard let clientId = WorkOSConfig.clientId else {
            return .failure(.invalidResponse(-1, "WorkOS client ID is not configured"))
        }
