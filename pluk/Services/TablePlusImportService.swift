...
/// and turn into a Pluk `Connection`.
...
enum TablePlusImportService {
    ...
    private static func candidate(from entry: [String: Any], groupNames: [String: String]) -> TablePlusImportCandidate {
        ...
        if databaseType == nil {
            unsupportedReason = "\(driverName) is not supported in Pluk yet"
        } else if usesSocket {
    ...
    /// TablePlus stores a pastel hex; pick the nearest Pluk palette color, and
