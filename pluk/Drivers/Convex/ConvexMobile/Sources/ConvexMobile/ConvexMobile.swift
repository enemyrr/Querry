...
public class ConvexClient {
  ...
  /// Executes a query with the given `name` and `args` and returns the raw result.
  ///
  /// This method provides direct access to the underlying query functionality,
  /// including system queries like `_system/cli/tables`.
  ///
  /// - Parameters:
  ///   - name: A value in "module:query_name" format that will be used when calling the backend
  ///   - args: A dictionary of arguments to be sent to the backend query function as JSON strings
  public func query(name: String, args: [String: String] = [:]) async throws -> String {
    return try await ffiClient.query(name: name, args: args)
  }

  /// Executes a query with the given `name` and `args` and returns the raw result.
  ///
  /// This method provides direct access to the underlying query functionality with ConvexEncodable support,
  /// matching the same API as the subscribe method.
  ///
  /// - Parameters:
  ///   - name: A value in "module:query_name" format that will be used when calling the backend
  ///   - args: A dictionary of arguments to be sent to the backend query function
  public func query(name: String, with args: [String: ConvexEncodable?]) async throws -> String {
    let stringArgs = try args.mapValues({ v in
      try v?.convexEncode() ?? "null"
    })
    return try await ffiClient.query(name: name, args: stringArgs)
  }

  /// Executes a query with the given `name` and `args` and returns the parsed JSON result as a dictionary.
  ///
  /// This method provides direct access to the underlying query functionality with automatic JSON parsing.
  ///
  /// - Parameters:
  ///   - name: A value in "module:query_name" format that will be used when calling the backend
  ///   - args: A dictionary of arguments to be sent to the backend query function
  public func query(name: String, with args: [String: ConvexEncodable?]) async throws -> [String: Any] {
    let response: String = try await query(name: name, with: args)
    
    guard let responseData = response.data(using: .utf8) else {
      throw ClientError.InternalError(msg: "Failed to convert response to UTF-8 data")
    }
    
    guard let json = try JSONSerialization.jsonObject(with: responseData) as? [String: Any] else {
      throw ClientError.InternalError(msg: "Response is not a valid JSON object")
    }
    
    return json
  }

  /// Sets admin authentication for use when calling Convex functions.
  ///
  /// Use this method with a deploy key from your Convex dashboard to perform
  /// administrative operations.
  ///
  /// - Parameters:
  ///   - deployKey: The deploy key from your Convex dashboard
  public func setAdminAuth(deployKey: String) async throws {
    try await ffiClient.setAdminAuth(deployKey: deployKey, actingAs: nil)
  }
...
public class ConvexClientWithAuth<T>: ConvexClient {
  ...
  /// Sets admin authentication bypassing the normal auth provider flow.
  ///
  /// Use this method with a deploy key from your Convex dashboard to perform
  /// administrative operations. This bypasses the normal user authentication
  /// flow and sets the client to use admin credentials directly.
  ///
  /// - Parameters:
  ///   - deployKey: The deploy key from your Convex dashboard
  ///   - actingAs: Optional JSON string representing user identity attributes to act as
  public func setAdminAuth(deployKey: String, actingAs: String? = nil) async throws {
    ...
    // Note: Admin auth bypasses the AuthProvider, so we don't update authState
  }
...
private class SubscriptionAdapter<T: Decodable>: QuerySubscriber {
  ...
  func onUpdate(value: String) {
    // For String output type, just send the value directly
    if Publisher.Output.self == String.self {
      publisher.send(value as! Publisher.Output)
    } else {
      // For other types, try to decode from JSON
