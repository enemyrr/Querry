#!/bin/bash
# sign-and-notarize.sh - Comprehensive code signing and notarization script for Pluk
# 
...
BUNDLE_DIR="build/Build/Products/Release/Pluk.app"
APP_BUNDLE_PATH="$APP_DIR/$BUNDLE_DIR"
ZIP_PATH="$APP_DIR/build/Pluk-notarize.zip"
FINAL_ZIP_PATH="$APP_DIR/build/Pluk-notarized.zip"
MAX_RETRIES=3
RETRY_DELAY=30
TIMEOUT_MINUTES=30
...
print_usage() {
    echo "Sign and Notarize Script for Pluk Mac App"
    echo ""
...
perform_signing() {
    log "Starting code signing process for Pluk..."
...
perform_notarization() {
    log "Starting notarization process for Pluk..."
...
log "Starting sign and notarize script for Pluk..."
