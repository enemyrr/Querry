...
# =============================================================================
# 
# This script builds the Pluk application using xcodebuild with optional
...
#
# OUTPUTS:
#   - Built app at: build/Build/Products/<Configuration>/Pluk.app
...
if [[ -f "Pluk/version.xcconfig" ]]; then
    EXPECTED_VERSION=$(grep 'MARKETING_VERSION' "Pluk/version.xcconfig" | sed 's/.*MARKETING_VERSION = //')
    EXPECTED_BUILD=$(grep 'CURRENT_PROJECT_VERSION' "Pluk/version.xcconfig" | sed 's/.*CURRENT_PROJECT_VERSION = //')
